...
class MyBuyOrderDetailView extends Component {
    constructor(props) {
        super(props);
        this.state = {
            order:{},
    ...
    onClickListener = (viewId) => {       
        if (viewId === 'confirm'){
            ...
            let smsNumber ='sms:'+ this.state.order.seller_id.phone_number;
            Linking.openURL(smsNumber);
        } else if (viewId === 'call') {
            let phoneNumber = '';
 
            if (Platform.OS === 'android') {
              phoneNumber = 'tel:'+ this.state.order.seller_id.phone_number;
            }
            else {
              phoneNumber = 'telprompt:'+this.state.order.seller_id.phone_number;
            }
         
            Linking.openURL(phoneNumber);
        } else if (viewId === 'whatsapp'){
            Linking.openURL('whatsapp://send?phone='+ this.state.order.seller_id.phone_number);
        }
    }
        ...
        this.state.carts.map((item, i) => {
          items.push(
            <ListItem
              ...
              onPress={() => this.onItemClick(item.product_id)}
            >
                <AsyncImageAnimated
                source={item.product_id.photos.length > 0 ? {uri: item.product_id.photos[0].src} : (require('./../../../../images/ic_logo_cir_red_small.png'))}
    ...
    render() {
        ...
        return (
            <View style={styles.container}>
                ...
                <Navbar left={left} title="Buy Detail" />
                <View style={{flex:1}}>
                    <View style={styles.container}>
...
export default MyBuyOrderDetailView;